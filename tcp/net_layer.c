...
bool updateRoutingTableSet(uint32_t* send_vip, uint32_t* dst_addr, uint32_t* dst_cost);
...
bool net_sendMessage(uint32_t from_vip_addr, uint32_t to_vip_addr, uint8_t protocol, char* data, size_t data_len)
{
    ...
    nxt_vip_addr_ptr = (uint32_t*)(g_hash_table_lookup(s_routing_table, &to_vip_addr));
    ...
    if (nxt_vip_addr_ptr == NULL) { // Next hop == NULL
        ...
        buildIPPacket(&ip_packet, to_vip_addr, from_vip_addr, protocol, data, data_len);
        ...
        value_ptr = g_hash_table_lookup(s_all_local_vips, &to_vip_addr);
...
bool net_handlePacketInput(int interface)
{
    ...
    memset(&ip_packet, 0, sizeof(ip_packet_t));
    ...
    // Forward packet
    bool result = sendIPPacket(&ip_packet, false);
    
    NET_PRINT("\nForwarding packet: ");
    printSendIPPacketResult(result, true);
    printf("dest: %s\n", util_convertVIPInt2String(dest_vip_addr));
    ...
    if (command == RIP_REQUEST_COMMAND) {//send data;
        ...
        while(num_entries_read<num_entries){
            ...
            *addr = (util_getU32((unsigned char*)data));
            ...
            updated_routes |= updateRoutingTableSet(sender_vip, addr, cost);
            num_entries_read++;
...
{
    uint32_t* nxt_hop_addr_ptr = NULL;
    uint32_t* local_addr_ptr = NULL;
    
    // Protect shared variable s_routing_table
    pthread_mutex_lock(&g_routing_table_mutex);
    nxt_hop_addr_ptr = g_hash_table_lookup(s_routing_table, &dest_addr);
    if (nxt_hop_addr_ptr == NULL) {
        pthread_mutex_unlock(&g_routing_table_mutex);
        return 0;
    }
    pthread_mutex_unlock(&g_routing_table_mutex);
    
    local_addr_ptr = g_hash_table_lookup(g_my_vip_from_sender, nxt_hop_addr_ptr);
    if (local_addr_ptr == NULL) {
        return 0;
    } else {
        return *local_addr_ptr;
    }
}
